...
- (instancetype)initWithConfiguration:(AWSServiceConfiguration *)configuration
    ...
    if (self = [super init]) {
        ...
        _keychain = [AWSKeyChainStore keyChainStoreWithService:[NSString stringWithFormat:@"%@.%@", [NSBundle mainBundle].bundleIdentifier, [AWSCognitoIdentityUserPool class]]];
        
        
        //If Pinpoint is setup, get the endpoint or create one.
        if(userPoolConfiguration.pinpointAppId) {
        
            AWSKeyChainStore *pinpointKeychain = [AWSKeyChainStore keyChainStoreWithService:AWSPinpointContextKeychainService];
