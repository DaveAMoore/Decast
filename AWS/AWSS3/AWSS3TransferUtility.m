            ...
            if ([obj isKindOfClass:[AWSS3TransferUtilityUploadTask class]])
            {
                ...
                if ([uploadTask.status isEqualToString:AWSS3TransferUtilityInProgressStatus]) {
                    ...
                    if ([task state] != NSURLSessionTaskStateRunning) {
                        ...
                        [self retryUpload:uploadTask retryCost:1];
                        continue;
                    }
                }
            }
               ...
               //Found a upload subtask.
                ...
                if ([task state] != NSURLSessionTaskStateRunning) {
                    ...
                    //We think the task in IN_PROGRESS. The underlying task is not running.
                    //Recover the situation by retrying.
                    [self retryUploadSubTask:multiPartUploadTask subTask:subTaskObj retryCost:1];
                }
            }
            else {
            ...
            if ([obj isKindOfClass:[AWSS3TransferUtilityDownloadTask class]])
            {
                ...
                if ([downloadTask.status isEqualToString:AWSS3TransferUtilityInProgressStatus]) {
                    ...
                    if ([task state] != NSURLSessionTaskStateRunning) {
                        ...
                        //Recover the situation by retrying
                        [self retryDownload:downloadTask retryCost:1];
                        continue;
                    }
                }
            }
...
-(AWSTask<AWSS3TransferUtilityUploadTask *> *) createUploadTask: (AWSS3TransferUtilityUploadTask *) transferUtilityUploadTask {
    ...
    return [[self.preSignedURLBuilder getPreSignedURL:getPreSignedURLRequest] continueWithSuccessBlock:^id(AWSTask *task) {
        ...
        //Add to Database
        [self insertUploadTransferRequestInDB:transferUtilityUploadTask databaseQueue:self.databaseQueue];
        [uploadTask resume];
...
- (void) retryUpload: (AWSS3TransferUtilityUploadTask *) transferUtilityUploadTask retryCost:(int)retryCost {
    ...
    transferUtilityUploadTask.retryCount = transferUtilityUploadTask.retryCount + retryCost;
...
-(void) retryUploadSubTask: (AWSS3TransferUtilityMultiPartUploadTask *) transferUtilityMultiPartUploadTask
    ...
    transferUtilityMultiPartUploadTask.retryCount = transferUtilityMultiPartUploadTask.retryCount + retryCost;
    [self createUploadSubTask:transferUtilityMultiPartUploadTask subTask:subTask startTransfer:YES];
}
...
- (void) retryDownload: (AWSS3TransferUtilityDownloadTask *) transferUtilityDownloadTask retryCost:(int)retryCost {
    ...
    transferUtilityDownloadTask.retryCount = transferUtilityDownloadTask.retryCount + retryCost;
...
- (AWSTask *)callFinishMultiPartForUploadTask:(AWSS3TransferUtilityMultiPartUploadTask *)uploadTask {
    ...
    NSMutableArray<AWSS3CompletedPart *> *completedParts = [NSMutableArray arrayWithCapacity:[uploadTask.completedPartsDictionary count]];
...
didCompleteWithError:(NSError *)error {
    ...
    if ([task isKindOfClass:[NSURLSessionUploadTask class]]) {
        ...
        if ([transferUtilityTask isKindOfClass:[AWSS3TransferUtilityUploadTask class]]) {
            ...
            if (error) {
                int retryCost = 0;
                if ([self shouldRetryWithError:error retryCost:&retryCost] || [self isErrorRetriable:HTTPResponse responseFromServer:uploadTask.responseData]) {
                    ...
                    if (uploadTask.retryCount < self.transferUtilityConfiguration.retryLimit) {
                        ...
                        [self retryUpload:uploadTask retryCost:retryCost];
                        return;
                    }
                }
            ...
            if (error) {
                ...
                int retryCost = 0;
                if ([self shouldRetryWithError:error retryCost:&retryCost] ||
                    [self isErrorRetriable:HTTPResponse responseFromServer:subTask.responseData]) {
                    ...
                    if (transferUtilityMultiPartUploadTask.retryCount < self.transferUtilityConfiguration.retryLimit) {
                        AWSDDLogDebug(@"Retry count is below limit and error is retriable. ");
                        [self retryUploadSubTask:transferUtilityMultiPartUploadTask subTask:subTask retryCost:retryCost];
                        return;
                    }
                }
        ...
        if (error) {
            int retryCost = 0;
            if ([self shouldRetryWithError:error retryCost:&retryCost] || [self isErrorRetriable:HTTPResponse responseFromServer:downloadTask.responseData]) {
                if (downloadTask.retryCount < self.transferUtilityConfiguration.retryLimit) {
                    AWSDDLogDebug(@"Retry count is below limit and error is retriable. ");
                    [self retryDownload:downloadTask retryCost:retryCost];
                    return;
                }
            }
...
- (BOOL)shouldRetryWithError:(NSError *)error retryCost:(int *)retryCost {
...
- (BOOL)isErrorRetriable:(NSHTTPURLResponse *)HTTPResponse responseFromServer:(NSString *)responseFromServer {
    if (!HTTPResponse) {
        return NO;
    }
            ...
            if (error && [error domain] == NSCocoaErrorDomain) {
                ...
                switch ([error code]) {
                    case NSFileNoSuchFileError:
                        [[NSFileManager defaultManager] createDirectoryAtURL:[transferUtilityTask.location URLByDeletingLastPathComponent]
                                                 withIntermediateDirectories:YES
                                                                  attributes:nil
                                                                       error:&anotherError];
                        break;
                    case NSFileWriteFileExistsError:
                        [[NSFileManager defaultManager] removeItemAtURL:transferUtilityTask.location
                                                                  error:&anotherError];
                ...
                if (!anotherError) {
                    ...
                    [[NSFileManager defaultManager] moveItemAtPath:filePath
                                                            toPath:transferUtilityTask.location.path
                                                             error:&anotherError];
                }
                
                transferUtilityTask.error = anotherError;
            } else {
