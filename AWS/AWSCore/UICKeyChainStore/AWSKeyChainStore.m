...
NSString * const AWSKeyChainStoreErrorDomain = @"com.kishikawakatsumi.AWSkeychainstore";
static NSString *_defaultService;

static dispatch_semaphore_t accessSemaphore;

@interface AWSKeyChainStore ()

@end

@implementation AWSKeyChainStore
...
+ (AWSKeyChainStore *)keyChainStoreWithService:(NSString *)service
{
    return [[self alloc] initWithService:service accessGroup:nil];
}

+ (AWSKeyChainStore *)keyChainStoreWithService:(NSString *)service accessGroup:(NSString *)accessGroup
{
...
{
    return [[self alloc] initWithServer:server protocolType:protocolType authenticationType:AWSKeyChainStoreAuthenticationTypeDefault];
}
...
- (instancetype)initWithService:(NSString *)service accessGroup:(NSString *)accessGroup
{
    self = [super init];
    if (self) {
        static dispatch_once_t accessSemaphoreOnce;
        dispatch_once(&accessSemaphoreOnce, ^{
            accessSemaphore = dispatch_semaphore_create(1);
        });
        
        _itemClass = AWSKeyChainStoreItemClassGenericPassword;
        
        if (!service) {
            service = [self.class defaultService];
        }
        
        // Modify the service depending on the type of build/
        NSString *buildType;
#if DEBUG
        buildType = @"Sandbox";
#else
        ...
        _service = [[service stringByAppendingFormat:@".%@", buildType] copy];
...
{
    return [self initWithServer:server protocolType:protocolType authenticationType:AWSKeyChainStoreAuthenticationTypeDefault];
}
...
- (instancetype)initWithServer:(NSURL *)server protocolType:(AWSKeyChainStoreProtocolType)protocolType authenticationType:(AWSKeyChainStoreAuthenticationType)authenticationType
{
    self = [super init];
    if (self) {
        _itemClass = AWSKeyChainStoreItemClassInternetPassword;
...
- (void)commonInit
{
    _accessibility = AWSKeyChainStoreAccessibilityAfterFirstUnlock;
...
+ (NSString *)stringForKey:(NSString *)key service:(NSString *)service accessGroup:(NSString *)accessGroup error:(NSError *__autoreleasing *)error
{
    ...
    AWSKeyChainStore *keychain = [AWSKeyChainStore keyChainStoreWithService:service accessGroup:accessGroup];
...
+ (NSArray AWS_KEY_TYPE *)allKeysWithItemClass:(AWSKeyChainStoreItemClass)itemClass
{
    CFTypeRef itemClassObject = kSecClassGenericPassword;
    if (itemClass == AWSKeyChainStoreItemClassGenericPassword) {
        itemClassObject = kSecClassGenericPassword;
    } else if (itemClass == AWSKeyChainStoreItemClassInternetPassword) {
...
+ (NSArray *)allItemsWithItemClass:(AWSKeyChainStoreItemClass)itemClass
{
    CFTypeRef itemClassObject = kSecClassGenericPassword;
    if (itemClass == AWSKeyChainStoreItemClassGenericPassword) {
        itemClassObject = kSecClassGenericPassword;
...
+ (void)requestSharedWebCredentialWithCompletion:(void (^)(NSArray AWS_CREDENTIAL_TYPE *credentials, NSError *error))completion
{
    [self requestSharedWebCredentialForDomain:nil account:nil completion:completion];
}

+ (void)requestSharedWebCredentialForDomain:(NSString *)domain account:(NSString *)account completion:(void (^)(NSArray AWS_CREDENTIAL_TYPE *credentials, NSError *error))completion
{
...
- (NSMutableDictionary *)attributesWithKey:(NSString *)key value:(NSData *)value error:(NSError *__autoreleasing *)error
{
    ...
    if (_authenticationPolicy && accessibilityObject) {
        ...
        if (floor(NSFoundationVersionNumber) <= floor(iOS_7_1_or_10_9_2) && _accessibility == AWSKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly) {
#if TARGET_OS_IOS
            NSLog(@"%@", @"Unavailable 'AWSKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly' attribute on iOS versions prior to 8.0.");
#else
            NSLog(@"%@", @"Unavailable 'AWSKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly' attribute on OS X versions prior to 10.10.");
#endif
        } else {
...
{
    switch (_itemClass) {
        case AWSKeyChainStoreItemClassGenericPassword:
            return kSecClassGenericPassword;
        case AWSKeyChainStoreItemClassInternetPassword:
...
- (CFTypeRef)protocolTypeObject
{
    switch (_protocolType) {
        case AWSKeyChainStoreProtocolTypeFTP:
            return kSecAttrProtocolFTP;
        case AWSKeyChainStoreProtocolTypeFTPAccount:
            return kSecAttrProtocolFTPAccount;
        case AWSKeyChainStoreProtocolTypeHTTP:
            return kSecAttrProtocolHTTP;
        case AWSKeyChainStoreProtocolTypeIRC:
            return kSecAttrProtocolIRC;
        case AWSKeyChainStoreProtocolTypeNNTP:
            return kSecAttrProtocolNNTP;
        case AWSKeyChainStoreProtocolTypePOP3:
            return kSecAttrProtocolPOP3;
        case AWSKeyChainStoreProtocolTypeSMTP:
            return kSecAttrProtocolSMTP;
        case AWSKeyChainStoreProtocolTypeSOCKS:
            return kSecAttrProtocolSOCKS;
        case AWSKeyChainStoreProtocolTypeIMAP:
            return kSecAttrProtocolIMAP;
        case AWSKeyChainStoreProtocolTypeLDAP:
            return kSecAttrProtocolLDAP;
        case AWSKeyChainStoreProtocolTypeAppleTalk:
            return kSecAttrProtocolAppleTalk;
        case AWSKeyChainStoreProtocolTypeAFP:
            return kSecAttrProtocolAFP;
        case AWSKeyChainStoreProtocolTypeTelnet:
            return kSecAttrProtocolTelnet;
        case AWSKeyChainStoreProtocolTypeSSH:
            return kSecAttrProtocolSSH;
        case AWSKeyChainStoreProtocolTypeFTPS:
            return kSecAttrProtocolFTPS;
        case AWSKeyChainStoreProtocolTypeHTTPS:
            return kSecAttrProtocolHTTPS;
        case AWSKeyChainStoreProtocolTypeHTTPProxy:
            return kSecAttrProtocolHTTPProxy;
        case AWSKeyChainStoreProtocolTypeHTTPSProxy:
            return kSecAttrProtocolHTTPSProxy;
        case AWSKeyChainStoreProtocolTypeFTPProxy:
            return kSecAttrProtocolFTPProxy;
        case AWSKeyChainStoreProtocolTypeSMB:
            return kSecAttrProtocolSMB;
        case AWSKeyChainStoreProtocolTypeRTSP:
            return kSecAttrProtocolRTSP;
        case AWSKeyChainStoreProtocolTypeRTSPProxy:
            return kSecAttrProtocolRTSPProxy;
        case AWSKeyChainStoreProtocolTypeDAAP:
            return kSecAttrProtocolDAAP;
        case AWSKeyChainStoreProtocolTypeEPPC:
            return kSecAttrProtocolEPPC;
        case AWSKeyChainStoreProtocolTypeNNTPS:
            return kSecAttrProtocolNNTPS;
        case AWSKeyChainStoreProtocolTypeLDAPS:
            return kSecAttrProtocolLDAPS;
        case AWSKeyChainStoreProtocolTypeTelnetS:
            return kSecAttrProtocolTelnetS;
        case AWSKeyChainStoreProtocolTypeIRCS:
            return kSecAttrProtocolIRCS;
        case AWSKeyChainStoreProtocolTypePOP3S:
...
- (CFTypeRef)authenticationTypeObject
{
    switch (_authenticationType) {
        case AWSKeyChainStoreAuthenticationTypeNTLM:
            return kSecAttrAuthenticationTypeNTLM;
        case AWSKeyChainStoreAuthenticationTypeMSN:
            return kSecAttrAuthenticationTypeMSN;
        case AWSKeyChainStoreAuthenticationTypeDPA:
            return kSecAttrAuthenticationTypeDPA;
        case AWSKeyChainStoreAuthenticationTypeRPA:
            return kSecAttrAuthenticationTypeRPA;
        case AWSKeyChainStoreAuthenticationTypeHTTPBasic:
            return kSecAttrAuthenticationTypeHTTPBasic;
        case AWSKeyChainStoreAuthenticationTypeHTTPDigest:
            return kSecAttrAuthenticationTypeHTTPDigest;
        case AWSKeyChainStoreAuthenticationTypeHTMLForm:
            return kSecAttrAuthenticationTypeHTMLForm;
        case AWSKeyChainStoreAuthenticationTypeDefault:
...
- (CFTypeRef)accessibilityObject
{
    switch (_accessibility) {
        case AWSKeyChainStoreAccessibilityWhenUnlocked:
            return kSecAttrAccessibleWhenUnlocked;
        case AWSKeyChainStoreAccessibilityAfterFirstUnlock:
            return kSecAttrAccessibleAfterFirstUnlock;
        case AWSKeyChainStoreAccessibilityAlways:
            return kSecAttrAccessibleAlways;
        case AWSKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly:
            return kSecAttrAccessibleWhenPasscodeSetThisDeviceOnly;
        case AWSKeyChainStoreAccessibilityWhenUnlockedThisDeviceOnly:
            return kSecAttrAccessibleWhenUnlockedThisDeviceOnly;
        case AWSKeyChainStoreAccessibilityAfterFirstUnlockThisDeviceOnly:
            return kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly;
        case AWSKeyChainStoreAccessibilityAlwaysThisDeviceOnly:
...
{
    NSError *error = [NSError errorWithDomain:AWSKeyChainStoreErrorDomain code:AWSKeyChainStoreErrorInvalidArguments userInfo:@{NSLocalizedDescriptionKey: message}];
...
{
    NSError *error = [NSError errorWithDomain:AWSKeyChainStoreErrorDomain code:-67594 userInfo:@{NSLocalizedDescriptionKey: message}];
    NSLog(@"error: [%@] %@", @(error.code), error.localizedDescription);
    return error;
}
...
#endif
    NSError *error = [NSError errorWithDomain:AWSKeyChainStoreErrorDomain code:status userInfo:@{NSLocalizedDescriptionKey: message}];
...
{
    NSError *error = [NSError errorWithDomain:AWSKeyChainStoreErrorDomain code:-99999 userInfo:@{NSLocalizedDescriptionKey: message}];
    NSLog(@"error: [%@] %@", @(error.code), error.localizedDescription);
    return error;
}
...
@implementation AWSKeyChainStore (Deprecation)
