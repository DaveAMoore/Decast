//
//  RFSinglePartSaveAssetOperation.swift
//  RFCore
//
...
/// An operation that uploads a single `RFRecord` in one whole segment of data.
internal class RFSinglePartSaveAssetOperation: RFContainerOperation {
    ...
    /// Transfer utility to use for saving the asset. Default value is derived through accessing `operationalContainer.transferUtility`.
    ...
    private var currentTask: AWSS3TransferUtilityUploadTask?
    ...
    open override func start() {
        ...
        let completionBlock: AWSS3TransferUtilityUploadCompletionHandlerBlock = { [weak self] task, error in
        ...
        // Start the upload.
        let uploadTask = transferUtility.uploadFile(asset.fileURL,
